...
def extract_spectogram(file_path, verbose=False):
    ...
    ffprobe_command = 'ffprobe "' + file_path + \
                      '" -show_entries stream=duration -select_streams a -of compact=p=0:nk=1 -v 0'
    duration = float(subprocess.check_output(ffprobe_command, universal_newlines=True).strip())
